...
class Rater():
    ...
    def score(self, y_true, y_pred, method='aucprc'):
        ...
        elif self.metric_ == 'aupr':
            return average_precision_score(y_true, y_pred)
        elif self.metric_ == 'auc':
            return roc_auc_score(y_true, y_pred)
        elif self.metric_ == 'gmean':
            y_pred_b = y_pred.copy()
            y_pred_b[y_pred_b < self.threshold_] = 0
            y_pred_b[y_pred_b >= self.threshold_] = 1
            return geometric_mean_score(y_true, y_pred_b)
        elif self.metric_ == 'f1_macro':
            y_pred_b = y_pred.copy()
            y_pred_b[y_pred_b < self.threshold_] = 0
            y_pred_b[y_pred_b >= self.threshold_] = 1
            return f1_score(y_true, y_pred_b, average='macro')
...
def meta_sampling(y_pred, y_true, X, n_under_samples, mu, sigma, random_state=None):
    ...
    sample_weights = gaussian_prob(np.abs(y_true - y_pred), mu, sigma)
    df_X = pd.DataFrame(X)  # 保留原始索引
    sampled_df = df_X.sample(n=n_under_samples, weights=sample_weights, random_state=random_state)

    # X_subset = sampled_df.values  # 采样后的数据（numpy 数组）
    X_subset = sampled_df
    sampled_indices = sampled_df.index.to_numpy()  # 采样到的原始索引

    return X_subset, sampled_indices
